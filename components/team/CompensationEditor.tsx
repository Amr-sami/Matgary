  ...
  if (!isOwner) {
    return (
      <div className="bg-white rounded-xl border border-border p-5 text-center text-text-secondary">
        إدارة الرواتب متاحة لصاحب المتجر فقط.
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center gap-2">
          ...
          <h3 className="font-bold text-base">رواتب الموظفين</h3>
        </div>
        <ul>
          {members.map((m) => (
...
function monthBounds(d: Date): { from: string; to: string; label: string } {
...
function PeriodSummary({ onToast }: { onToast: (t: Toast) => void }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (!teamRes.ok) throw new Error("تعذر تحميل الفريق");
    ...
    <div className="bg-accent-light/30 border border-accent-light rounded-xl p-5 space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-base">ملخص الرواتب — {period.label}</h3>
        </div>
        <Button
          variant="secondary"
          onClick={exportCsv}
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Stat label="عدد الموظفين" value={members.length.toString()} />
            <Stat
              label="إجمالي الراتب"
          ...
          {totalReview > 0 && (
            <div className="flex items-start gap-2 p-2.5 rounded-lg bg-warning/10 border border-warning/30">
              ...
              <p className="text-xs text-text-primary leading-relaxed">
                هناك <strong>{totalReview}</strong> فترة تحتاج مراجعة (نسيان
                تسجيل الانصراف). راجعها من تبويب "الحضور" قبل اعتماد الراتب.
              </p>
            </div>
          )}
        </>
...
function MemberRow({
  ...
  const submit = async () => {
    ...
    try {
      ...
      await loadHistory();
      onToast({ type: "success", message: "تم حفظ الراتب" });
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-text-secondary truncate">
              {current ? summarizeCompensation(current) : "لم يتم تحديد راتب"}
            </p>
          </div>
        </div>
      ...
      {isOpen && (
        <div className="px-5 pb-5 space-y-4 bg-bg-main/30">
          <Select
            ...
            options={[
              { value: "fixed", label: "راتب شهري ثابت" },
              { value: "hourly", label: "بالساعة فقط" },
              { value: "hybrid", label: "راتب أساسي + إضافي بالساعة" },
            ]}
          />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {payType === "hybrid" && (
              <Input
                label="الساعات المشمولة في الراتب الأساسي / شهر"
                type="number"
                inputMode="numeric"
                value={standardHours}
            ...
            <div className="pt-4 border-t border-border">
              ...
              <ul className="space-y-1.5">
                {history.slice(1).map((h) => (
                  ...
                  >
                    <span>{summarizeCompensation(h)}</span>
                    <span className="shrink-0">
                      {new Date(h.effectiveFrom).toLocaleDateString("ar-EG")}
                    </span>
                  </li>
                ))}
              </ul>
...
function summarizeCompensation(c: CompensationRow): string {
  if (c.payType === "fixed") {
    return `راتب شهري ${formatPrice(c.baseSalaryMonthly ?? 0)}`;
  }
  if (c.payType === "hourly") {
    return `${formatPrice(c.hourlyRate ?? 0)} / ساعة`;
  }
  return `أساسي ${formatPrice(c.baseSalaryMonthly ?? 0)} + ${formatPrice(c.hourlyRate ?? 0)}/ساعة`;
}
